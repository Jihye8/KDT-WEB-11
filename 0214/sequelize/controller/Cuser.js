...
//회원정보 전체
// findAll은 배열형태로 결과 출력
exports.Call = async (req, res) => {
    Member.findAll({
        // attributes : 원하는 컬럼 조회
        // id 5에 해당하는 정보의 username과 userid 만 출력
        attributes: ['id', 'username', 'userid'],
        // Op.gt : 초과, Op.gte : 이상,  Op.lt(미만), Op.lte : 이하
        // Op.or : 또는, Op.ne : 같지 않음, Op.in(배열 요소 중 하나), Op.notIn(배열요소와 모두 다름)
        where: { id: { [Op.gt]: 4 }, order: [['id', 'DESC']] },
        limit: 2,
        offset: 2,
    }).then((result) => {
        console.log(result);
...
exports.Cupdate = async (req, res) => {
    ...
    Member.update({ username }, { password: pw }, { where: { id } }).then(
        (result) => {
